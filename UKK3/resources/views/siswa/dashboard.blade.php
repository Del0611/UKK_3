...
<head>
    ...
    <style>
        ...
        .clickable-text { cursor: pointer; transition: 0.2s; padding: 4px; border-radius: 4px; display: block; }
        .clickable-text:hover { background: rgba(255,255,255,0.05); color: #0dcaf0 !important; }
    </style>
</head>
<body class="antialiased">
    ...
    <nav class="navbar navbar-expand-lg fixed-top shadow-sm">
        <div class="container">
            <div class="d-flex align-items-center gap-3">
                ...
                <a class="navbar-brand fw-bold text-uppercase m-0" href="#" style="letter-spacing: -1px;">
                    Student<span class="text-info">Hub</span>
                </a>
            </div>
            <div class="d-flex align-items-center gap-4">
                <div class="d-none d-md-flex flex-column align-items-end">
                    <span class="text-info fw-bold text-uppercase" style="font-size: 10px; letter-spacing: 1px;">Siswa Aktif</span>
                    <span class="fw-bold text-light small">{{ $data['username'] }}</span>
                </div>
                <a href="/logout-siswa" class="btn btn-outline-danger btn-sm fw-bold text-uppercase px-4 py-2 rounded-3" style="font-size: 11px;">Keluar</a>
            </div>
        </div>
    </nav>
    ...
    <main class="container" style="margin-top: 110px; padding-bottom: 50px;">
        <header class="py-4">
            <h1 class="fw-bold text-white display-6 mb-1">Halo, {{ $data['username'] }}!</h1>
        ...
        <div class="row g-4">
            <div class="col-12 col-lg-4">
                <div class="card-custom p-4 shadow-sm h-100">
                    <h5 class="fw-bold text-white text-uppercase mb-4" style="font-size: 14px; letter-spacing: 1px;">Kirim Aspirasi Baru</h5>
                    <form action="/aspirasi-simpan" method="POST">
                        @csrf
                        @if(session('success'))
                            <div class="alert alert-success border-0 small py-2 mb-3 shadow-sm">{{ session('success') }}</div>
                        @endif
                        ...
                        <div class="d-grid">
                            <button type="submit" class="btn btn-info fw-bold text-uppercase py-2" style="color: #0b1120;">Kirim Sekarang</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="col-12 col-lg-8">
                <div class="card-custom shadow-sm h-100 overflow-hidden">
                    <div class="p-4 border-bottom border-secondary border-opacity-10 bg-info bg-opacity-10">
                        <h6 class="m-0 fw-bold text-uppercase small text-info" style="letter-spacing: 1px;">Riwayat Aspirasimu</h6>
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-dark text-secondary">
                                <tr style="font-size: 10px; letter-spacing: 1px;">
                                    ...
                                    <th class="px-4 py-3 border-0">PESAN (KLIK UNTUK DETAIL)</th>
                            ...
                            <tbody class="text-light border-top-0">
                                ...
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                                    <td class="px-4 py-4 small text-nowrap">
                                        <div class="text-secondary"><i class="bi bi-calendar-event me-1"></i>{{ date('d M Y', strtotime($r->created_at)) }}</div>
                                        @if($r->updated_at && $r->updated_at != $r->created_at)
                                            <div class="text-info mt-1" style="font-size: 10px; opacity: 0.8;"><i class="bi bi-clock-history me-1"></i>Update: {{ date('d M H:i', strtotime($r->updated_at)) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 small">
                                        <div class="clickable-text" onclick="showDetailSiswa(this)" data-pesan="{{ $r->ket }}" data-lokasi="{{ $r->lokasi }}">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            @if($r->status == 'pending')
                                                <button class="btn btn-outline-info btn-action" onclick="openEditModal({{ json_encode($r) }})">
                                            ...
                                            @else
                                                <button class="btn btn-secondary btn-action opacity-25" disabled title="Sedang diproses">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-dark border-secondary shadow-lg">
                <div class="modal-header border-secondary">
                    <h5 class="modal-title fw-bold text-info">Detail Aspirasi</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p class="small text-secondary fw-bold text-uppercase mb-1">Lokasi:</p>
                    <p id="detLokasi" class="text-white fw-bold mb-3"></p>
                    <p class="small text-secondary fw-bold text-uppercase mb-1">Pesan Lengkap:</p>
                    <div id="detPesan" class="p-3 bg-black bg-opacity-50 rounded border border-secondary text-light" style="white-space: pre-wrap;"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function showDetailSiswa(el) {
            const pesan = $(el).attr('data-pesan');
            const lokasi = $(el).attr('data-lokasi');
            
            $('#detLokasi').text(lokasi);
            $('#detPesan').text(pesan);
            
            var detailMdl = new bootstrap.Modal(document.getElementById('detailModal'));
            detailMdl.show();
        }
    </script>
</body>
