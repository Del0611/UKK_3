...
session_start();
  ...
  $yanggan_nomor_pensiun = $_GET['nomor_pensiun'];
}
   if (isset($_POST['yanggan'])) {
  // Ambil data yang dikirim dari formulir registrasi  
  $yanggan_status = $_POST['status'];
  $yanggan_keterangan = $_POST['keterangan'];

 

  // Query untuk menyimpan data pengguna baru

     $sql = "UPDATE tb_peserta SET status = '$yanggan_status', keterangan = '$yanggan_keterangan' WHERE nomor_pensiun = '$yanggan_nomor_pensiun'";
...
<body>
  ...
  <td>
  <input type="radio" id="Valid" name="status" value="21">
  ...
  <td>
    <input type="radio" id="TidakValid" name="status" value="22">
  ...
  <td>
      ...
      <input type="submit" name="yanggan" value="Kirim">
  </td>
  </tr>
  </table>
  </div>
